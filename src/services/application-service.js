...
async function getApplicationByName(vid, vkey, applicationName) {
  core.debug(
    `Module: application-service, function: getApplicationByName. Application: ${applicationName}`,
  );
  const resource = {
...
async function getVeracodeSandboxIDFromProfile(vid, vkey, appguid) {
  core.debug(
    `Module: application-service, function: getSandboxIDfromProfile. Application: ${appguid}`,
  );
  const resource = {
...
async function createSandboxRequest(vid, vkey, appguid, sandboxname) {
  core.debug(
    `Module: application-service, function: createSandbox. Application: ${appguid}`,
  );
  const resource = {
    resourceUri: appConfig().applicationUri + "/" + appguid + "/sandboxes",
    resourceData: {
...
function profileExists(responseData, applicationName) {
  core.debug(
    `Module: application-service, function: profileExists. Application: ${applicationName}`,
  );
  if (responseData.page.total_elements === 0) {
    ...
    for (let i = 0; i < responseData._embedded.applications.length; i++) {
      if (
        responseData._embedded.applications[i].profile.name.toLowerCase() ===
    ...
    core.debug(
      `No Veracode application profile with exact the profile name: ${applicationName}`,
    );
    return { exists: false, veracodeApp: null };
  }
}

async function getVeracodeApplicationForPolicyScan(
  vid,
  vkey,
  applicationName,
  policyName,
  ...
  core.debug(
    `Module: application-service, function: getVeracodeApplicationForPolicyScan. Application: ${applicationName}`,
  );
  ...
  if (!profile.exists) {
    ...
    core.debug(
      `Create Veracode application profile: ${JSON.stringify(resource)}`,
    );
    const response = await createResource(vid, vkey, resource);
    core.debug(
      `Veracode application profile created: ${JSON.stringify(response)}`,
    );
...
async function getVeracodeApplicationScanStatus(
  vid,
  vkey,
  veracodeApp,
  buildId,
  ...
  if (sandboxID > 1) {
    ...
    const output = await runCommand("java", [
      "-jar",
      jarName,
      "-vid",
      vid,
    ...
    core.info(
      "Veracode Scan Status: " +
        result.buildinfo.build[0].analysis_unit[0].att.status
          .replace(/ /g, "_")
          .toUpperCase(),
    );
    core.info(
      "Veracode Policy Compliance Status: " +
        result.buildinfo.build[0].att.policy_compliance_status
          .replace(/ /g, "_")
          .toUpperCase(),
    );
    core.info(
      "Veracode Scan Date: " +
        result.buildinfo.build[0].analysis_unit[0].att.published_date,
    );
    return {
...
async function getVeracodeApplicationFindings(
  vid,
  vkey,
  veracodeApp,
  buildId,
  ...
  const resultsUrlBase =
    "https://analysiscenter.veracode.com/auth/index.jsp#ViewReportsResultSummary";
    ...
    if (jsonData.page.total_elements > 0) {
      ...
      for (i = 0; i <= findings.length - 1; i++) {
        if (findings[i].finding_status.status != fixedSearchTerm) {
          console.log(
            "Finding " +
              JSON.stringify(findings[i].issue_id) +
              " is not open and will be ignored",
          );
          console.log(
            "Finding status: " +
              JSON.stringify(findings[i].finding_status.status),
          );
        } else {
          //adding finding to new array
          console.log(
            "Finding " + JSON.stringify(findings[i].issue_id) + " is open",
          );
          console.log(
            "Finding status: " +
              JSON.stringify(findings[i].finding_status.status),
  ...
  const files = ["policy_flaws.json", "results_url.txt"];
