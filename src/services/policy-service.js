...
async function getVeracodePolicyByName(vid, vkey, policyName) {
  core.debug(
    `Module: policy-service, function: getVeracodePolicyByName. policyName: ${policyName}`,
  );
  if (policyName !== "") {
    ...
    if (responseData.page.total_elements !== 0) {
      for (let i = 0; i < responseData._embedded.policy_versions.length; i++) {
        if (
          responseData._embedded.policy_versions[i].name.toLowerCase() ===
  ...
  core.debug(
    `No Veracode policy found for ${policyName}, using default policy`,
  );
