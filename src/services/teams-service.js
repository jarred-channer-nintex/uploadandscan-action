...
const { getResourceByAttribute } = require("../api/http-requests.js");
...
async function getVeracodeTeamsByName(vid, vkey, teams) {
  core.debug(
    `Module: teams-service, function: getVeracodeTeamsByName. teams: ${teams}`,
  );
  if (teams !== "") {
    ...
    for (let index = 0; index < teamsName.length; index++) {
      ...
      if (responseData.page.total_elements !== 0) {
        for (let i = 0; i < responseData._embedded.teams.length; i++) {
          if (
            responseData._embedded.teams[i].team_name.toLowerCase() ===
