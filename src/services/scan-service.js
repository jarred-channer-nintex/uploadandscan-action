...
async function createBuild(
  ...
  if (output === "failed" && deleteincompletescan === "false") {
    ...
    const deleteOutput = await runCommand("java", [
      "-jar",
      jarName,
      "-vid",
      vid,
...
async function createSandboxBuild(
  vid,
  vkey,
  jarName,
  appId,
...
async function getModules(vid, vkey, jarName, appId, include, sandboxID) {
  ...
  modulesToScan.forEach((moduleName) => {
    modules.forEach((m) => {
      if (
        m.name &&
        minimatch(m.name.toLowerCase(), moduleName.trim().toLowerCase())
      ) {
        moduleIds.push(m.id);
      }
    });
  ...
  if (outputXML.indexOf("Results Ready") > -1) {
    ...
    result.buildinfo.build.forEach((build) => {
      if (build.build_id === buildId) {
        if (build.$.policy_compliance_status === "Calculating...")
